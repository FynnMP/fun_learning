...
class MemoryTile:
        ...
        if not self.first:
            ...
            if self.answer[row][column] == self.answer[a][b]:
                ...
                if not any(''.join(row) for row in self.answer):
                    ...
                    if duration < 90:  # Check if duration is less than 90 seconds
                        self.score += 500  # Increment additional score for completing in under 90 seconds
                        self.score_label.config(text='Gewinn CHF: {}'.format(self.score))  # Update the score label
                    message = 'You win! Time: {:.1f}\n\nDo you want to play again?'.format(duration)
                    play_again = messagebox.askyesno(title='Success!', message=message)
                    if play_again:
                        self.draw_board()
                    else:
                        self.parent.quit()  # Schließt das Tkinter-Fenster und beendet das Spiel
    ...
    def hide_tiles(self, x1, y1, x2, y2):
        ...
        self.score = max(0, self.score - 20)  # Subtract 20 from the score, ensuring it doesn't go below 0
        self.score_label.config(text='Gewinn CHF: {}'.format(self.score))  # Update the score label
